...
| 도입 기술        | 개선 효과                                 |
|------------------|---------------------------------------|
| Docker           | “환경 일관성을 유지하며 개발–배포 간격을 줄이기 위해 도입, 개발-운영 간 일관성을 확보함. 도커 이미지 기반 배포로 반복 가능한 배포 환경을 구축함”    |
| Swagger          | “API 명세를 코드로부터 자동 생성하여 문서 관리 부담을 줄이고, 테스트 UI 제공으로 프론트/백엔드 협업 속도 향상” |
| Postman          | “세션 기반 로그인 등 복잡한 API 흐름을 쉽게 시뮬레이션하고, 에러 케이스 재현을 자동화된 테스트 시나리오로 구성할 수 있음”    |
| GitHub Actions   | “빌드-테스트-도커 이미지 푸시까지 자동화하여 개발자가 코드 푸시에만 집중해도 배포 가능한 흐름을 구축함”         |
<br>
